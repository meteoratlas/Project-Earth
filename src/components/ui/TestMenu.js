...
class TestMenu extends Component {
  ...
  sendInput = async (e) => {
    for (const each of this.commands) {
      ...
      if (each.command === "idArrowDown") this.props.moves(0, -each.amount);
      if (each.command === "idArrowLeft") this.props.moves(-each.amount, 0);
      if (each.command === "idArrowRight") this.props.moves(each.amount, 0);
  ...
  addToTable = (e) => {
    ...
    // if table is full
    if (this.nextAvailIndex === this.limitMove) return alert("The table hits maximum move");
    ...
    if (this.nextAvailIndex < this.limitMove) {
      ...
      document.getElementsByClassName("move")[this.nextAvailIndex].textContent = command;
      document.getElementsByClassName("detail")[this.nextAvailIndex].textContent = detail;
